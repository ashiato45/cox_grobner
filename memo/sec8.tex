    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \begin{enumerate}
          ...
          1と3より、
          \begin{align}
            a_0 = a_0(0,0)\cdot 1^m + \dots + \ub{a_m(0,0)}_{斉次多項式なので0} = f(0,0,1)  \neq 0.
          \end{align}
        \end{enumerate}
        ...
        \item $\exists$:
        iとiiとiiiを命題3-6-3を適用し、$w\in \C$が存在して、$f,g$が$(u,v,w)$で消える。
        \footnote{
...
\begin{myproof}
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      $t\mapsto f(\varphi(t)a + (1-\varphi(t))b)$を考える。
      \item
      「$f\in \C[x_1,\dots,x_n]$が0でないなら、
      $\C^n - \var(f)$は弧状連結である」を示す。
      $a,b \in\C^n - \var(f)$とし、それを結ぶ複素直線
      $t\mapsto f(ta + (1-t)b)$を考える。
      上は$t$の多項式として0でないので、(a)を適用して、
      $\C-\var(t\mapsto f(ta+(1-t)b))$に含まれる、
      $t=0$と$t=1$を結ぶpathが求まることになる。
      そのpathを$\varphi\colon [0,1]\to \C-\var(t\mapsto f(ta+(1-t)b))$
      とする。path$\varphi$が$\C-\var(t\mapsto f(ta+(1-t)b))$に
      含まれるので、$t\in [0,1]$について、
      $(t\mapsto f(ta+(1-t)b))(\varphi(t)) \neq 0$であり、
      $f(\varphi(t)a + (1-\varphi(t))b) \neq 0$であり、
      $t\mapsto f(\varphi(t)a + (1-\varphi(t)b))$
      は$[0,1]$上のどこでも非0である。よって、
      $\C^n$上のpath$t\mapsto \varphi(t) a + (1-\varphi(t))$を考えると、
      これは$[0,1]$上のどこでも$f$を0にしない。よって、
      pathは$\C^n-\var(f)$に含まれる。pathが求まり、弧状連結である。
