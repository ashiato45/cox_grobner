...
グレブナ基底とS多項式の余りとの関係を見たが、これをグレブナ基底とS多項式の0への簡約を見る。「$\set{g_1,\dots,g_s}$がグレブナ基底である$\iff$ $i\neq j$について、$S(g_i,g_j) \toG 0$」
\begin{myproof}
  \begin{itemize}
    \item $\Rightarrow$:
    ...
    \begin{align}
      ...
      \sum_{m(i)<\delta}h_i g_i
    \end{align}
    が得られた。これの$m'$を計算する。まず第1の総和を計算する。
    \begin{align}
      m'(新しい表現の第1の総和)
      &=
      m'(\sum_{m(i)=\delta} \sum_{i,j,k=1}^s \frac{(\LT(h_j)g_j)\vee (\LT(h_k)g_k)}{g_j\vee g_k} a_{ijk}g_i)\\
      &\fbox{足し算をしたほうが小さくなる}\\
      &=
      \max_{i,j,k}\deg(\frac{(\LT(h_j)g_j)\vee (\LT(h_k)g_k)}{g_j\vee g_k} a_{ijk}g_i)\\
      &\fbox{足し算したほうが小さくなる}\\
      &=
      \max_{j,k}\deg(\frac{(\LT(h_j)g_j)\vee (\LT(h_k)g_k)}{g_j\vee g_k} S(g_j,g_k))\\
      &\fbox{S多項式の計算}\\
      &=
      \max_{j,k}\deg(S(\LT(h_j)g_j, \LT(h_k)g_k)\\
      &\fbox{同次を足したら次数が真に落ちてS多項式で書ける}\\
      &<
      \deg(\sum_{m(i)=\delta} \LT(h_i)g_i)\\
      &=
      \delta.
    \end{align}
    第2の総和、第3の総和はそもそも$m'$は$\delta$未満なので、結局
    \begin{align}
      m'(新しい表現) < \delta
    \end{align}
    となる。
...
% 割り算することなく$S(f,g) \toG 0$かどうかを判定する方法の1つを見る。
...
% となっている。
%
% 分解の基本になる特殊なシチギーを定義する。あるシチギー$S\in S(F)$が
% 多重次数$\alpha$次斉次であるとは、
% \begin{itemize}
...
これで、グレブナ基底の判定条件を$\toG 0$を使って拡張する。
「イデアル$I$の基底$G=(g_1,\dots,g_t)$がグレブナ基底である
$\iff$
\begin{align}
  \Exists{S_1,\dots,S_m はS(G)の斉次な基底}
  \Forall{i=1,\dots,m}
  S_i G \toG 0.
\end{align}
」
\begin{myproof}
  \begin{itemize}
    \item $\Rightarrow$:$S(G)$の斉次な基底として$S_{ij}$全体をとっておく。
    すると、$S_{ij}G\toG 0$を確かめればよいが、これは$\ovd{S(g_i,g_j)}{G}=0$
    を示せばよく、これは先のS多項式とグレブナ基底の関係である。
    \item $\Leftarrow$:
    $f\in I$とする。$\LT(f)\in \gen{\LT(G)}$を示せばグレブナ基底であることが示せる。
    $f\in I = \gen{G}$なので、$f = \sum_{i=1}^t h_i g_i$となる$h_\bullet \in k[x_1,\dots,x_n]$が存在する。$m\colon \set{1,\dots,t} \to \Zge^n$を
    $m(i) = \deg(h_i g_i)$とする。
    そして、$m'\colon (g_\bullet の線形結合の表現) \to \Zge^n$を
    \begin{align}
      m'(\sum_{i=1}^t a_i g_i) = \max_{i=1,\dots,t} \deg(a_ig_i)
    \end{align}
    と定義する。そして、$\delta = m'(\sum_{i=1}^t h_i g_i)$とする。
    $\delta$が最小になるように$h_\bullet$選んでおくことができるので、
    そうしておく。
    $f=\sum_{i=1}^t h_i g_i$だったので、$\deg(f)\le \delta$である。
    ここで、仮に$\deg(f)<\delta$だったとして矛盾を導こう。

    \begin{align}
      f = \sum_{m(i)=\delta} \LT(h_i)g_i
      +
      \sum_{m(i)=\delta} (h_i - \LT(h_i))g_i
      +
      \sum_{m(i)<\delta} h_i g_i
    \end{align}
    と書ける。$\deg(f)<\delta$と仮定したので、第1の総和の$\sum_{m(i)=\delta}\LT(h_i)g_i$の$m'$は$\delta$未満でなければならない。
    このためには、この和のなかで先頭項の打ち消しが起こらなければならないので、
    $\sum_{m(i)=\delta}\LT(h_i)\LT(g_i)=0$とならなければならない。
    よって、
    \begin{align}
      S = \sum_{m(i)=\delta} \LT(h_i)\bbold e_i
    \end{align}
    とすると、これは$S(G)$に属するシチギーとなる。
    これは0でないところは単項式になっているので、$\delta$次斉次のシチギーになっていることがわかる。仮定より、斉次な基底$S_1,\dots,S_m$が存在するので、
    \begin{align}
      S = \sum_{j=1}^m u_i S_i
    \end{align}
    となる$u_i \in k[x_1,\dots,x_n]$が存在する。$S$が$\delta$次斉次であり、
    $S_j$らも斉次なので、係数を比較することにより$u_j S_j$が\footnote{「$S_j$が」ではない。}$\delta$次斉次でないなら、
    $u_j = 0$である\footnote{それならはじめから書かなきゃいいじゃないかという気がするが、基底が来るのが$f$が来るより先なので仕方無い。}。
    よって、$S=\sum_{u_j \neq 0} u_j S_j$である。$u_j S_j$は$G$の$\delta$次斉次なシチギーなので、
    \begin{align}
      \deg((u_j S_j)G) < \delta
    \end{align}
    と、真に次数が落ちる。
    さらに、次数の条件の準備をする。仮定より、$S_j G \toG 0$なので、
    \begin{align}
      S_j G = \sum_{i=1}^t a_{ij} g_i
    \end{align}
    なる$a_{ij}\in k[x_1,\dots,x_n]$が存在し、さらに次数の条件
    \begin{align}
      \deg(a_{ij}g_i) \le \deg(S_j G)
    \end{align}
    がかかる。
    ...
    \begin{align}
      ...
      SG
      =
      (\sum_{j=1}^m u_j S_j)G
      =
      \sum_{u_j\neq 0}u_j \sum_{i=1}^t a_{ij}g_i
    \end{align}
    なので、新たな$f$の表現
    \begin{align}
      f= \sum_{u_j \neq 0}u_j \sum_{i=1}^t a_{ij}g_i
      ...
      \sum_{m(i)<\delta}h_i g_i
    \end{align}
    が得られる。これの$m'$を計算する。まず第1の総和について計算する。
    \begin{align}
      m'(新しい表現の第1の総和)
      &=
      m'( \sum_{u_j \neq 0}u_j \sum_{i=1}^t a_{ij}g_i)\\
      &=
      \max_{i,j}m(u_j a_{ij}g_i)\\
      &\fbox{$m'$の定義}\\
      &\le
      \max_{i,j}m(u_j S_j G)\\
      &\fbox{次数の条件}\\
      &<
      \delta\\
      &\fbox{$u_j S_j$は$\delta$次斉次のシチギー}.\\
    \end{align}
第2、第3の総和についてはそもそも$m'$は$\delta$未満である。

よって、
$m'(新しい表現) < \delta$がわかった。これは、はじめに表現を最小に取っておいたことに矛盾する。よって、$\deg(f)=\delta=\max_{i}\deg(h_ig_i)$である。最大を与える$i$を
$I$とすると、$\deg(f)=\deg(h_I g_I)$となり、$\LT(g_I)|\LT(f)$となる。よって、
$\LT(f)\in \gen{\LT(G)}$となり、$\LT(I) \subset \gen{\LT(G)}$となる。
よって、$\gen{\LT(I)} \subset \gen{\LT(G)}$となり、$G$はグレブナ基底である。
  \end{itemize}
\end{myproof}

シチジーの基底から余計なものを取り除く条件を考える。シチジーの基底は
ペアのシチジーで張れることがさっきわかったので、そのときを考えてみる。
「$G=(g_1,\dots,g_s)$とする。
$\cal S \subset \set{S_{ij}; 1\le i < j \le n}$とし、
$\cal S$が$S(G)$を張るとする。さらに、相異なる$i,j,k$について、
\begin{itemize}
  \item $\LT(f_k)| (f_i \vee f_j)$
  \item $S_{ik},\, S_{jk} \in \cal S$
\end{itemize}
となっているとする。このとき、$\cal S - \set{S_{ij}}$も$S(G)$の基底になっている。」
\begin{myproof}
  $S_{ij}$が$S_{ik}$と$S_{jk}$で書ければよい。$i < j < k$としておく。
  \begin{align}
    S_{ij}
    &=
    \frac{f_i \vee f_j}{\LT(f_i)}\bbold e_i - \frac{f_i \vee f_j}{\LT(f_j)}\bbold e_j\\
    &=
    \frac{f_i \vee f_j}{f_i \vee f_k}\cdot \frac{f_i \vee f_k}{\LT(f_i)}\bbold e_i
    -
    \frac{f_i \vee f_j}{f_j \vee f_k}\cdot \frac{f_j \vee f_k}{\LT(f_j)}\bbold e_j\\
    &=
    \frac{f_i \vee f_j}{f_i \vee f_k}(\frac{f_i\vee f_k}{\LT(f_i)}\bbold e_i - \frac{f_i \vee f_k}{\LT(f_k)}\bbold e_k + \frac{f_i \vee f_k}{\LT(f_k)}\bbold e_k)
    -
    \frac{f_i \vee f_j}{f_j \vee f_k}(\frac{f_j \ve f_k}{\LT(f_j)}\bbold e_j -
    \frac{f_j \vee f_k}{\LT(f_k)}\bbold e_k + \frac{f_j \vee f_k}{\LT(f_k)}\bbold e_k)\\
    &=
    \frac{f_i \vee f_j}{f_i \vee f_k}S_{ik} - \frac{f_i \vee f_j}{f_j \vee f_k}S_{jk}.
  \end{align}
\end{myproof}
...
% \begin{myproof}
%   \begin{itemize}
%     \item どの段階でも$B$に含まれるペアについて、それに対応する$f_\bullet$が
%     $G$に含まれること:
%     初期ではあきらか。$G$は増えるだけなことに注意。そのため、$B$が減少することは問題にならない。
%     $B$が増えるときも、増やした$G$にあわせている。
%     \item どの段階でも$B$が次の性質を持つことを示す:
%     \begin{align}
%       \begin{cases}
%         1 \le i < j \le t \\
%         かつ \\
...
%       \end{cases}
%     \end{align}
%     いままで同様、メインループ前の変数は$b$を、後は$a$をつける。
%     初期では$(i,j)\in B$がどの$i,j$についてもみたされ、前件が否定されるので自明に成立する。$B_b$が条件
%     \begin{align}
%       \begin{cases}
%         1 \le i_b < j_b \le t_b \\
%         かつ \\
...
%       \implies
%       \begin{cases}
%         S(f_{i_b},f_{j_b}) \rightarrow_{G_b} 0 \\
%         または \\
%         \Criterion(f_{i_b},f_{j_b},B_b)
%       \end{cases}
%     \end{align}
%     を満たしているとし、さらに
%     \begin{align}
%       1\le i_a < j_a \le t_a \quad かつ \quad (i_a,j_a)\notin B_a
%     \end{align}
%     が満たされているとして、
%     \begin{align}
%       S(f_{i_a},f_{j_a}) \rightarrow_{G_a} 0 \quad または
%       \quad
%       \Criterion(f_{i_a},f_{j_a},B_a)
%     \end{align}
%     を示そう。
%     \begin{enumerate}[label=(\alph*)]
%       \item $(i_a,j_a) \in B_b$のとき:
%       $(i_a,j_a)\notin B_a$でかつ$(i_a,j_a)\in B_b$となっている。
%       これは、$B$のプログラム中の遷移を見ると、プログラム中で$(i,j)$
%       として$(i_a,j_a)$がえらばれている。
%       この$(i_a,j_a)$を追跡する。
%       \begin{enumerate}[label=(\Alph*)]
%         \item \MARU{1}でYesのとき:
%         \begin{align}
...
%         \end{align}
%         である。
%          このときは$\LT(f_{i_a})$と$\LT(f_{j_a})$が互いに素である。
%
%          このとき、「どの段階でも$B$に含まれるペアについて、それに対応する$f_\bullet$が
%          $G$に含まれる」と$(i_a,j_a)\in B_b$より、$f_{i_a},f_{j_a} \in G_b = G_a$である。よって、先の命題により
%          $S(f_{i_a},f_{j_a}) \to_{G_a} 0$である。
%         \item \MARU{2}でYesのとき:
%         \begin{align}
%           B_a &= B_b - \set{(i_a,j_a)}\\
%           t_a &= t_b \\
...
%         である。
%
%         自明に$\Criterion(f_{i_a},f_{j_a},B_a)$が成立する。
%         \item \MARU{3}でYesのとき:
%         \begin{align}
%           B_a &= B_b - \set{(i_a,j_a)}\\
%           t_a &= t_b \\
%           G_a &= G_b
%         \end{align}
%         である。このとき、「どの段階でも$B$に含まれるペアについて、それに対応する$f_\bullet$が
%         $G$に含まれる」と$(i_a,j_a)\in B_b$より、$f_{i_a},f_{j_a} \in G_b = G_a$である。よって、
%         \begin{align}
%           \ovd{S(f_{i_a},f_{j_a})}{G_a} = \ovd{S(f_{i_a},f_{j_a})}{G_b}= S= 0
%         \end{align}
%         となる。よって、$S(f_{i_a},f_{j_a}) \to_{G_a} 0$である。
%         \item \MARU{3}でNoのとき:
%         \begin{align}
%           t_a &= t_b + 1 \\
%           f_{t_a} &= S = \ovd{S(f_{i_a},f_{j_a})}{G_b}\\
%           G_a &= G_b + [f_{t_a}]\\
%           B_a &= B_b \cup \set{(i,t_{a}) ; 1\le i \le t_b} - \set{(i_a,j_a)}\\
%         \end{align}
%         となっている。このとき、$q_\bullet$を商として、
%         \begin{align}
%           S(f_{i_a},f_{j_a}) = \sum_{I=1}^{t_b} q_I f_I + \ovd{S(f_{i_a},f_{j_a})}{G_b}
%         \end{align}
%         となっている。さらにすすめると、
%         \begin{align}
%           S(f_{i_a},f_{j_a}) = \sum_{I=1}^{t_b} q_I f_I + f_{t_a}
%         \end{align}
%         となっており、$S(f_{i_a},f_{j_a})$を$G_a$で割ったあまりが0になることがわかる。よって、$S(f_{i_a},f_{j_a}) \to_{G_a} 0$である。
%       \end{enumerate}
%       \item $(i_a,j_a) \notin B_b$のとき:
%       $(i_a,j_a)\notin B_a$でかつ$(i_a,j_a)\notin B_b$となっている。
%       何か$i<j$なる$(i,j)\in B_b$がアルゴリズム中で選択されて、
%       \begin{align}
%         B_a = B_b - \set{(i,j)}
%       \end{align}
%       となっている。$(i,j)\in B_b$でかつ$(i_a,j_a)\notin B_b$なので、
...
%           G_a &= G_b
%         \end{align}
%         となる。$1\le i_a < j_a \le t_a = t_b$となり、かつ$(i_a,j_a) \notin B_b$なので、$B_b$の条件が満たされ、
%         \begin{align}
%           S(f_{i_a},f_{j_a}) \to_{G_b} 0 \quad
%           または \quad
%           \Criterion(f_{i_a},f_{j_a},B_b)
%         \end{align}
%         $S(f_{i_a},f_{j_a}) \to_{G_b} 0$がみたされるなら$G_b = G_a$より
%         $S(f_{i_a},f_{j_a}) \to_{G_a} 0$となり、示される。
%
%         $\Criterion(f_{i_a},f_{j_a},B_b)$のときを考える。
%         このとき、
%         \begin{itemize}
%           \item $[i_a,k] \notin B_b$
%           \item $[j_a,k] \notin B_b$
%           \item $\LT(f_k)|(f_{i_a}\vee f_{j_a})$
%         \end{itemize}
%         のすべてをみたす$k \in \set{1,\dots,t_b}-\set{i_a,j_a}$が存在する。
%         $B_a = B_b -\set{(i_a,j_a)}$なので、
%         $B_a \subset B_b$なので、$B_b^c \subset B_a^c$である。
%         $[i_a,k] \in B_b^c \subset B_a^c$となり、$[i_a,k]\notin B_a$となる。
%         同様に$[j_a,k] \notin B_a$となる。また、$t_a = t_b$なので、
%         $k\in \set{1,\dots,t_a}-\set{i_a,j_a}$である。まとめると、$k$が
%         \begin{itemize}
%           \item $[i_a,k]\notin B_a$
%           \item $[j_a,k]\notin B_a$
%           \item $\LT(f_k)|(f_{i_a} \vee f_{j_a})$
%           \item $k\in \set{1,\dots, t_a}-\set{i_a,j_a}$
%         \end{itemize}
%         をみたす。よって、$\Criterion(f_{i_a},f_{j_a},B_a)$が満足される。
%         \item \MARU{2}がYesのとき:
%         \begin{align}
%           B_a &= B_b - \set{(i,j)}\\
%           t_a &= t_b \\
%           G_a &= G_b
%         \end{align}
%         なので、「$\MARU{1}$がYesのとき」と同様に通る。
%         \item \MARU{3}がYesのとき:
%         やはり同様。
%         \item \MARU{3}がNoのとき:
%         このとき、
%         \begin{align}
%           S &= \ovd{S(f_{i},f_{j})}{G_b}\\
%           t_a &= t_b + 1\\
%           f_{t_a} &= S = \ovd{S(f_{i},f_{j})}{G_b}\\
%           G_a &= G_b + [f_{t_a}]\\
%           B_a &= (B_b \cup \set{(I,t_a); 1\le I \le t_b})-\set{(i,j)}
%         \end{align}
%         となっている。
%         \begin{enumerate}[label=(\roman*)]
%           \item $j_a = t_a$のとき:
%           2番目の要素を見ると、あきらかに$(i_a,j_a) = (i_a,t_a) \in B_b \cup \set{(I,t_a); 1 \le I \le t_b}$である。また、$(i_a,j_a) = (i_a,t_a) \neq (i,j)$であることを先に言ったので、
%           \begin{align}
%             (i_a,j_a) \in (B_b \cup \set{(I,t_a); 1 \le I \le t_b}) - \set{(i,j)}
%             =
%             B_a
%           \end{align}
%           である。一方、(b)に入ったときの仮定により、$(i_a,j_a)\notin B_a$である。
%           これは矛盾であるから、この(a-D-i)という状況は起こらない。
%           \item $j_a < t_a$のとき:
%             このとき、$B_b$の条件を満たすので、
%             \begin{align}
%               S(f_{i_a},f_{j_b})\to_{G_b} 0 \quad
%               または \quad
%               \Criterion(f_{i_a},f_{j_a},B_b)
%             \end{align}
%             となる。$S(f_{i_a},f_{j_a})\to_{G_b} 0$を満たすときには、
%             $G_a$は$G_b$より広いので$S(f_{i_a},f_{j_a})\to_{G_a}0$となり、示される。
%
%             $\Criterion(f_{i_a},f_{j_a},B_b)$が満たされるときは、
%             \begin{itemize}
%               \item $[i_a,k]\notin B_b$
%               \item $[j_a,k]\notin B_b$
%               \item $\LT(f_k)|(f_{i_a}\vee f_{j_a})$
%             \end{itemize}
%             のすべてをみたす$k\in \set{1,\dots,t_b}-\set{i_a,j_a}$が存在する。
%             仮に$[i_a,k]\in B_a$とする。$i_a$も$k$も$\set{1,\dots,t_b}$の元なので、
%             $\set{(I,t_a); 1\le I \le t_b}$には属さない。よって、$B_b$に属する。よって、$[i_a,k]\notin B_a$がわかった。同様に、
%             $[j_a,k]\notin B_a$もわかる。引き続き
%             $\LT(f_k)|(f_{i_a}\vee f_{j_a})$もみたされるので、
%             $\Criterion(f_{i_a}f_{j_a},B_a)$も満たされる。
%         \end{enumerate}
%       \end{enumerate}
%     \end{enumerate}
%     \item $B=\emptyset \implies Gはグレブナ基底$:
%     $t$を$G$の長さとする。
%     \begin{align}
%       \cal I =
%       \set{(i,j); 1\le i < j \le t,\quad \Criterion(f_i,f_j,B)は(i,j)が選択されたときfalse.}
%     \end{align}
%     とする。このとき、あとで証明する主張がなりたつ:
%     「$\cal S = \set{S_{ij}; (i,j)\in \cal I}$は『$S_{ij}G = S(f_i,f_j)\toG 0$がすべての$S_{ij}\in \cal S$について成立する』という性質を持つ$S(G)$の基底である。」
%
%     $\cal S$を考えると、これは$S_{ij}$だけでできた基底なので斉次基底であり、さらに
%     「$S_{ij}G \toG 0$がすべての$S_{ij}\in \cal S$について成立する」という性質を持つので、先のグレブナ基底の$\toG 0$での判定の定理を利用し、$G$がグレブナ基底であることがわかる。
%
%     $B=\emptyset$はなんだったのかという気がするが、それはこの「主張」を示すのに使う。
%     \item 「主張」の証明:$B\neq \emptyset$とする。
%     「どの段階でも$B$が次の性質を持つことを示す」と$B\neq \emptyset$より、前件が自明に満足されるので、
%     \begin{align}
%       1 \le i < j \le t
%       \implies
%       \begin{cases}
...
%       \end{cases}
%     \end{align}
%     となる。$\cal I$の定義より、$(i,j)$について、
%     $\Criterion(f_i,f_j,B)=false$がその$B$において成立するので、「または」の一方が潰され、$S(f_i,f_j)\toG 0$が成立する。
%
%     あとは、$\cal S$が$S(G)$の基底になることを示す。
%     $list$を、$B$から$(i,j)$のペアを外したときのものを順に入れたものにする。
%     次に、その$list$を逆順に並べ替える。このはじめの時点では、
%     $S_{ij}$をすべて含んでいるので、$S(G)$を張る。
%     この$list$を先頭から走って、$\Criterion(f_i,f_j,B_{(i,j)が選択されたとき})$が$true$なら除去するという作業をくりかえす。取り除くとき、
%     $\Criterion(f_i,f_j,B_{(i,j)が選択されたとき})$が$true$なので、
%     「何か$k\notin \set{i,j}$が存在して、これに対して
%     \begin{itemize}
%       \item $[i,k]\notin B$
%       \item $[j,k] \notin B$
%       \item $\LT(f_k)|(f_i \vee f_j)$
%       」となっている。この時点で$[i,k]\notin B$で$[j,k]\notin B$なので、
%       $list$が逆順になっていることから、$[i,k],[j,k]\in list$となっている。
%       そして、$\LT(f_k)|(f_i \vee f_j)$となっているので、
%       「シチジーの基底から余計なものを取り除く条件」より、
%       除去したあとでも$S(G)$を張る。この操作を$list$を最後まで走ると、
%       $\cal I$となる。よって、$\cal I$が所与の性質を持つ。
%     \end{itemize}
%   \item アルゴリズムの停止:
%   停止しなかったとしよう。このとき、$B$は毎回減少するので、
%   停止しないためには永遠に$B$の追加が行なわれなければならず、これに伴なって$G$は
%   余りで増やされていく。余りの定義より、$\gen{\LT(G)}$は真に増加し続けることになるが、これは多項式環がACCを満たすことに矛盾する。
%
%   \end{itemize}
% \end{myproof}

% \begin{handcopy}
%   アルゴリズムの基本的なアイディアは、$B$に考慮すべきペア$(i,j)$を記録しておくことである。
%   さらに、我々は命題4も命題10も適用できないS多項式$S(g_i,g_j)$に対してだけ余りを計算すれっばよい。
%
%   このアルゴリズムが有効に機能することを証明するために、まず、アルゴリズムのすべての段階において、$B $が次の性質を持つことを観察する。すなわち、もし
%   $1\le i<j\le t$かつ$(i,j)\notin B$であるならば、
%   \begin{align}
%     S(f_i,f_j)\toG 0 あるいは \Criterion(f_i,f_j,B)が成り立つ
%   \end{align}
%   初期状態ではこれはただしい。なぜなら$B$はすべての可能なペアを持った集合として始まるからである。我々は、$B$のある中間の状態に対して(8)が成り立っているならば、
%   $B$の状態が変化して(それを例えば$B'$とする)も
%   引き続いて(8)が成り立っていることを示す必要がある。
%
%   これを証明するために、$(i,j)\notin B'$としよう。もし$(i,j)\in B$ならば、
%   アルゴリズムを調べると$B'=B-\set{(i,j)}$となることがわかる。
%   さて、$B$から$(i,j)$を取り除く前のステップを見よう。もし、
%   $\LCM(\LT(f_i),\LT(f_j))=\LT(f_i)\LT(f_j)$であるならば、
%   命題4により$S(f_i,f_j)\toG 0$で(8)は成り立つ。また、
%   $\Criterion(f_i,f_j,B)$が真であるならば(8)は明らかに成り立つ。そこで、どちらも真でなかったと仮定しよう。この場合は、アルゴリズムは余り$S=\ovd{S(f_i,f_j)}{G}$を計算する。もし、$S=0$ならば、補題2より$S(f_i,f_j)\toG 0$という期待していた結果が出る。
%   再度に、もし$S\neq 0$であるならば、$G$を$G'=G\cup \set{S}$まで拡張する。
%   すると$S(f_i,f_j)\to_{G'}0$となるが、これが正しいことの検証は読者にまかせる。
%
%   しかしまだ$(i,j)\notin B$の場合を調べることがん怒っている。ここで(8)は$B$に対しては成立する。これから(8)が$B'$に対しても成立することが導かれるが、この証明は読者の演習問題として残す。
%
%   次に、$B=\emptyset$であるときも、$G$がグレブナ基底であることを示す必要がある。これを証明するために、$t$を$G$の長さとし、次の性質を見たすペア$(i,j) (1\le i<j\le t)$全体の集合$\cal I$を考える: 判定条件$\Criterion(f_i,f_j,B)$はペア$(i,j)$がアルゴリズムの中で選択されたとき偽である。このとき次の主張が成り立つ:$\cal S=\set{S_{ij}:(i,j)\in \cal I}$は、$S_{ij}G = S(f_i,f_j)\toG 0$がすべての
%   $S_{ij}\in \cal S$に対して成り立つという性質を持つ、$S(G)$の来手いである。この主張と定理9によって、$G$がグレブナ基底であることが証明される。
%
%   主張を証明するために、(8)が$1\le i < j\le t$であるすべてのペア$(i,j)$に対して成り立つことが、$B=\emptyset$から導かれることに注意する。これより、すべての$(i,j)\in \cal I$に対して$S(f_i,f_j)\toG 0$であることが従う。残ったのは、$\cal S$が$S(G)$の基底になることを証明することである。これを証明するために、まず、アルゴリズムにおいて、いつそれが$B$から取り除かれたかに従って、ペア$(i,j)$を順序付けることができることに注意しよう(この順序付けの詳細については演習問題10を参照)。さて、最後に取り除かれたものから出発して、順序を逆にしてペアの順序集合をたどる。そして、アルゴリズムのその時点で$\Criterion(f_i,f_j,B)$が真となるペア$(i,j)$を取り除いていく。すべてのペアを通過して、取り除くべきものは取り除いたあとの集合に残ったペアがちょうど$\cal I$
%   の元になる。このアルゴリズム実行する各段階において、まだ取り除かれていないペア
%   $(i,j)$に対応するシチギーは$S(G)$の基底を作ることを示す。
%   $S_{ij}$全体は$S(G)$の基底となり、初期状態はすべての$S_{ij}$から始めているので、この主張はアルゴリズムの初期状態では正しい。さらに、ある時点でペア$(i,j)$を取り除いたとすれば、$\Criterion(f_i,f_j,B)$の定義から、$k$が存在して$\LT(f_k)$は
%   LCM条件を満たし、$[i,k],[j,k]\notin B$であることが導かれる。こうして、
%   $[i,k]$と$[j,k]$は早いうちに$B$から取り除かれ、したがって、$S_{ik}$と$S_{jk}$は今計算している集合にまだのこっている。なぜならペアの集合を逆の順序でたどっているからである。命題10より、$S_{ij}$を消した後でも残った元が基底となっていることが従う。
%
%   最後に、アルゴリズムが停止することを証明する必要がある。もとのアルゴリズムの証明と同様に(sec 7の定理2)、$G$はいうでもイデアルの基底である。元を付け加えて$G$を拡張するたびに、単項式イデアル$(\gen{\LT(G)})$は真に大きくなる。昇鎖条件ACC
%   により、いずれかの時点で$G$はそれ以上おおきくならない。したがって、
%   最終的に$B$には元は加えられなくなる。アルゴリズムのWHILE...DOループを通過するたびに
%   $B$の元が1つ取り除かれるので、最終的には$B=\emptyset$とならなければならず、ここでアルゴリズムは停止する。
%   \end{handcopy}
%

命題4について。
「有限集合$G\subset k[x_1,\dots,x_n]$を考える。$f,g\in G$が
\begin{align}
  \LCM(\LM(f),\LM(g)) = \LM(f)\cdot \LM(g)
\end{align}
を満たしているとする。これは、$f$と$g$の先頭単項式が互いに素であることを意味している。
このとき$S(f,g)\toG 0$である。」
\begin{myproof}
  \begin{enumerate}[label=(\arabic*)]
    \item 簡単のために、$\LC(f)=\LC(g)=1$とする。
    \item Define $p$ as $f = \LM(f) + p$.
    \item Define $q$ as $g = \LM(g) + q$.
    \item
    \begin{align}
      S(f,g) &=
      \LM(g) f - \LM(f)g\\
    ...
    \item Set a subgoal:
    \begin{align}
      \multideg(S(f,g)) = \max(\multideg(pg),\multideg(qf)) ?
    \end{align}
    \item Because of (4) and "multidegree of a sum $\le$ multidegree of terms"
    \begin{align}
      \deg(S(f,g)) \le \max(\deg(pg), \deg(qf)).
    \end{align}
    \item Make assumption: (6) satisfies $<$.
    \item Because (4) and (6), (4)'s LT vanishes, and $\LT(pg) = \LT(qf)$.
    And $\LT(p)\LT(g) = \LT(q) \LT(f)$.
    \item Because of (8), $\LT(f)$ divides $\LT(g)\LT(p)$.  Assumption $\LT(f)\perp \LT(g)$ implies $\LT(f)$ divides $\LT(p)$.
    \item Definition (2) means $\deg(\LT(p)) < \deg(\LT(f))$, but this contradicts (9).
    \item Contradiction (10) negates assumption (7), and we get
    \begin{align}
      \deg(S(f,g)) = \max(\deg(pg), \deg(qf)).
    \end{align}
    This completes subgoal (5).
    \item We get a representation (4) $S(f,g)= pg-qf$, and (11) satisfies "contraction by G to 0"'s degree condition $\deg(S(f,g)) \ge \deg(pg), \deg(qf)$.  $S(f,g) \toG 0$.
  \end{enumerate}
\end{myproof}

補題7: 「$S(F)$のすべての元は、$S(F)$の斉次な元の和に一意に分解できる。」
\begin{myproof}
  \begin{enumerate}[label=(\arabic*)]
    \item Let $S=(h_1,\dots,h_s) \in S(F)$.  (This means $SF=0$.)
    \item Fix $\alpha \in \Zge^n$.
    \item Fix $i$.
    \item Fix $h_{i\alpha}$ a term of $h_i$, and $\deg(h_{i\alpha}f_i) = \alpha$.
    \item $h_{i\alpha}\LT(f_i)$ is the term of $\sum_{i=1}^s h_i \LT(f_i) = 0$ whose degree is $\alpha$.
    \item Free $i$ fixed in (3).
    \item Take terms whose degrees are $\alpha$ of $\sum_{i=1}^s h_i \LT(f_i)=0$, and we get $\sum_{i=1}^s h_{i\alpha}\LT(f_i) = 0$.
    \item Free $\alpha$ fixed in (2).  $\alpha$ runs over $\Zge^n$, so $h_i = \sum_{\alpha \in \Zge^n} h_{i\alpha}$.
    $S= (\sum_{\alpha \in \Zge^n} h_{i\alpha})_{i} = \sum_{\alpha\in \Zge^n} (h_{i\alpha})_i$.  For each $i$, $(h_{i\alpha})$ are syzygys whose multidegree are $\alpha$.
  \end{enumerate}
\end{myproof}

命題8:$F=(f_1,\dots,f_s)$を考える。すべてのシチギー$S\in S(F)$は
\begin{align}
  S=\sum_{i<j} u_{ij}S_{ij}
\end{align}
とかける。ここで、$u_{ij} \in k[x_1,\dots,x_n]$で、シチギー$S_{ij}$は
\begin{align}
  S_{ij} = \frac{x^{\gamma}}{\LT(f)}\bbold e_i - \frac{x^\gamma}{\LT(g)}\bbold e_j
  ,\quad
  x^\gamma = \LCM(\LT(f),\LT(g))
\end{align}
である。
\begin{myproof}
  \begin{enumerate}[label=(\arabic*)]
      \item Fix $S\in S(F)$.
      \item Assume $S$ has degree of $\alpha$.  This also means $S \neq 0$.
      \item Because $S$ is a syzygy, so $S$ has at least 2 non-zero components.  Let their indexes are $i < j$, and let $S[i] = c_i x^{\alpha(i)},\, S[j] = c_j x^{\alpha(j)}$ where $c_i, c_j \in k,\, \alpha(i), \alpha(j) \in \Zge^n$.
      Without loss of generarity, we can assume that $\alpha(i) \ge \alpha(j)$.
      \item Because of (2), $\alpha(i) + \deg(f_i) = \alpha$ and $\alpha(j) + \deg(f_j) = \alpha$.
      \item Because of (4),
      \begin{align}
          \alpha - \gamma
          =
          \alpha - \max(\alpha(i),\alpha(j))
          =
          \alpha - \alpha(j)
          =
          \multideg(f_j)
          ...
          0.
      \end{align}
      So, $x^{\alpha-\gamma}$ is polynomial (not fractional function).  Using this fact, calculate $S' = S-c_i \LC(f_i)x^{\alpha - \gamma}S_{ij}$.  Its $i$-th component is
      \begin{align}
        S'[i]
        &=
        S[i] - (c_i \LC(f_i)x^{\alpha-\gamma}S_{ij})[i]\\
        &=
        c_i x^{\alpha(i)} - c_i \LC(f_i)x^{\alpha-\gamma} \frac{x^{\gamma}}{\LT(f_i)}\\
        &=
        0.
      \end{align}
      \item The $i$-th component of $S'$ is zero, and zeros in $S$ are also zero in $S'$, so zeros of $S'$ is less than $S$.
      \item The fact that syzygy $S(F)$ is $k[x_1,\dots,x_n]$ module implies that $S'$ is also syzygy, because $S, S_{ij}$ are syzygy of $\LT(F)$.
      \item (6)(7) implies that $S'$ is a syzygy, and has less zeros than $S$.
      Procedure from (3) to here is repeated until $S$ become $0$.
      This means that $S$ is written as $k[x_1,\dots,x_n]$-coefficient linear combination of $(S_{ij})_{ij}$.  This completes the proof.
  \end{enumerate}
\end{myproof}

定理9:「イデアル$I$の基底$G=(g_1,\dots,g_t)$がグレブナ基底であることと、
シチギー全体の集合$S(G)$において、その1つの斉次基底のすべての元
$S=(h_1,\dots,h_t)$に対して
\begin{align}
  SG = \sum_{i=1}^t h_i g_i \toG 0
\end{align}
となることは同値である。」
\begin{myproof}
  \begin{enumerate}[label=(\arabic*)]
    \item Fix $f\in S(G)$.  $f$ can be represented as $f = \sum_{i=1}^t h_i g_i$ using $h_i \in k[x_1,\dots,x_n]$.
    \item Define $m(i) = \deg(h_i g_i)$.
    \item Define $\delta = \max(m(i); i=1,\dots,t)$.
    \item Retake $h_1,\dots,h_t$ in (1) $\delta$ to be smallest.
    \item (1) and definition (3) implies $\deg(f) \le \delta$.
    \item Assume that $\deg(f) < \delta$.
    \item (6) implies that the cancellation of leading terms in $\sum_{m(i)=\delta} \LT(h_i)g_i$ occurs.
    \item (7) implies $\sum_{m(i)=\delta} \LT(h_i)\LT(g_i) = 0$.
    \item Define $S=\sum_{m(i)=\delta} \LT(h_i)\bbold e_i$.
    \item Because of (8), $S \in S(G)$.
    \item Because of definition (2), when $m(i)=\delta$, the multidegree of $S$ is $\delta$.
    \item By assumption, there exists homogeneous syzygy basis $S_1,\dots,S_m \in S(G)$, and $S_j G \toG 0$ holds for all $j$.
    \item Because of (12), $S$ is written as $S=u_1 S_1 + \dots + u_m S_m$ using $u_1,\dots, u_m \in k[x_1,\dots,x_n]$.
    \item Because $S$ is homogeneous syzygy of multidegree $\delta$,
    for all $i$ $u_i$ is zero or monomial of degree $\delta - \multideg(S_j)$.
    \item In the situation (14), for all $i$, $u_j = 0$ or $u_j S_j$ is homogeneous syzygy of degree $\delta$.
    \item Define $\gamma_j = \multidegree(S_j) = \delta - \multidegree(u_j)$.
    \item Define $u_j = c_j x^{\delta-\gamma_j}$ for all $j$ such that $u_j \neq 0$.  Because of (16), $x^{\delta-\gamma_j}$ is exactly monomial.
    \item Because (13), (15) and (17) holds,
    \begin{align}
      S = \sum_{u_j \neq 0} c_j x^{\delta-\gamma_j}S_j.
    \end{align}
    \item From (18),
    \begin{align}
      \sum_{m(i)=\delta}\LT(h_i) g_i = SG = \sum_j c_j x^{\delta-\gamma_j} S_j G.
    \end{align}
    \item Because of (12), forall $i,j$, there exists $a_{1j},\dots,a_{tj} \in k[x_1,\dots,x_n]$ such that $S_j G = \sum_{i=1}^t a_{ij} g_i$, and
    \begin{align}
      \deg(a_{ij} g_i) \le \deg(S_j G)
    \end{align}
    holds.
    \item Forall $j$, $S_j$ is a syzygy, so $\deg(S_j G) < \delta$.
    \item
    \begin{align}
      \max_{ij}\deg(a_{ij}g_i)
      &\descle{(20)}
      \max_{j} \deg(S_j G)\\
      & \desclt{(21)}
      \delta.
    ...
    \item
    \begin{align}
      \sum_{m(i)=\delta}\LT(h_i)g_i \desceq{(19)} \sum_j c_j x^{\delta-\gamma_j}S_j G
      \desceq{(20)}
      \sum_j c_j x^{\delta^\gamma_j} \sum_{i=1}^t a_{ij}g_i.
    \end{align}
    So (23) and this equation means we get a new representation of $f$ of new $delta$ which is smaller than $\delta$ defined in (3).
    \item This contradicts to (4), so $\delta = \deg(f)$.
    \item This means $\LT(f)$ equals to some $h_i g_i$, so $\LT(g_i)|\LT(f)$.
    \item This means $\LT(f) \in \gen{\LT(G)}$, so $G$ is Groebner basis.  This completes the proof.
  \end{enumerate}
\end{myproof}

命題10: 「$G=(g_1,\dots,g_t)$を考える。部分集合$\cal S\subset \set{S_{ij}: 1\le i < j \le t}$が$S(G)$の基底になっていると仮定する。さらに、異なった$g_i,g_j,g_k \in G$が条件
『$\LT(g_k)$は$\LCM(\LT(g_i),\LT(g_j))$を割り切る』を満たしているとするこのとき、もし$S_{ik},S_{jk} \in \cal S$ならば$\cal S-\set{S_{ij}}$もまた$S(G)$の基底である
(注意:もし$i>j$ならば$S_{ij}=S_{ji}$とおく)。」
\begin{myproof}
  \begin{enumerate}[label=(\arabic*)]
    \item Fix $g_i,g_j,g_k \in G$.
    \item Without loss of generarity, assume $i < j < k$.
    \item Define $\gamma_{ij}$ as
    \begin{align}
      x^{\gamma_{ij}} = \LCM(\LM(g_i),\LM(g_j)).
    \end{align}
    Define $\gamma_{ik},\gamma_{jk}$ similary.
    \item
    \begin{align}
      S_{ij}
      &=
      \frac{x^{\gamma_{ij}}}{\LT(g_i)}\bbold e_i - \frac{x^{\gamma_{ij}}}{\LT(g_j)}\bbold e_j\\
      &=
      \frac{x^{\gamma_{ij}}}{x^{\gamma_{ik}}}\frac{x^{\gamma_{ik}}}{\LT(g_i)}\bbold e_i
      -
      \frac{x^{\gamma_{ij}}}{x^{\gamma_{jk}}}\frac{x^{\gamma_{jk}}}{\LT(g_j)}\bbold e_j\\
      &=
      \frac{x^{\gamma_{ij}}}{x^{\gamma_{ik}}}S_{ik} - \frac{x^{\gamma_{ij}}}{x^{\gamma_{jk}}}S_{jk}.
    \end{align}
    In this calculation, $x^{\gamma_{ij}}/x^{\gamma_{ik}}$ is polynomial because assmption $\LT(g_k)|x^{\gamma_{ij}}$.  $x^{\gamma_{ij}}/x^{\gamma_{jk}}$ is also polynomial.
  \end{enumerate}
\end{myproof}

% 定理11: $I=\gen{f_1,\dots,f_s}$を多項式イデアルとする。$I$のグレブナ基底は、次のアルゴリズムによって有限回のステップで構成できる。～アルゴリズム～
% ここで、$\Criterion(f_i,f_j,B)$は次の条件が満たされるとき真である:
% 「$k\notin \set{i,j}$が存在して、これに対して$[i,k]$と$[j,k]$は$B$に属さず、
% さらに$\LT(f_k)$は$\LCM(\LT(f_i),\LT(f_j))$を割り切る。
%
% アレンジ版を書いてみる。
% \begin{myproof}
%   \begin{algorithm}[H]
%     \begin{algorithmic}[1]
%       \STATE{$B\defeq \set{(i,j); 1 \le i < j \le s}$}
%       \STATE{ $G\defeq F$ }
%       \STATE{ $TRASH\defeq []$}
%       \STATE{ $BASIS \defeq []$}
%       \WHILE{ $B\neq \emptset$}
%         \STATE{Choose $(i,j)\in B$}
%         \IF{$Criterion(i,j,TRASH,BASIS) = true$}
%           \STATE{Move $(i,j)$ from $B$ to $TRASH$}
%         \ELSE{}
%           \IF{$f_i$と$f_j$が互いに素でない}
%             \STATE{$S \Leftarrow \ovd{S(f_i,f_j)}{G}$}
%             \IF{$S\neq 0$}
%               \STATE{Add $S$ to $G$}
%               \STATE{Add $(1,t+1),\dots,(t,t+1)$ to $B$ where $t+1$ is the last of $G$.}
%             \ENDIF{}
%           \ENDIF{}
%           \STATE{Move $(i,j)$ from $B$ to $BASIS$}
%         \ENDIF{}
%       \ENDWHILE{}
%     \end{algorithmic}
%   \end{algorithm}
%   $\Criterion(i,j,TRASH,BASIS,G)$は、$g_k \in G$なる$k$が存在して、
%   \begin{itemize}
%     \item $k$は$i$でも$j$でもない。
%     \item $[i,k] \in TRASH \cup BASIS$
%     \item $[j,k] \in TRASH \cup BASIS$
%     \item $\LT(g_k)| \LCM(\LT(g_i),\LT(g_j))$
%   \end{itemize}
%   を満たすこととする。
%   \begin{itemize}
%     \item $\set{(i,j); 1\le i < j \le t}= B\sqcup BASIC \sqcup TRASH$:あきらか。
%     \item $BASIS \cup B$は$S(G)$の基底である:
%     上のbulletより、$TRASH$は$BASIS \cup B$でかけることを言えばよい。
%     \begin{itemize}
%       \item L.17が実行されるとき:$BASIS\cup B$も$TRASH$も不変である。
%       $G$も変化しないならばあきらかなので、
%       L.13,14が実行されるときだけを考えればよい。
%       このとき、$G$に追加されたのは$\ovd{S(f_i,f_j)}{G}$であり、
%       \begin{align}
%         G_a = G_b \cup \set{\ovd{S(f_i,f_j)}{G_b}}
%       \end{align}
%       である。さらに、
%       \begin{align}
%         B_a = B_b \cup \set{(1,t+1),\dots,(t,t+1)}
%       \end{align}
%       である。
%       \begin{align}
%         BASIS_a \cup B_a
%         &=
%         (BASIS_b \cup \set{(i,j)}) \cup ((B_b\cup \set{(1,t+1),\dots,(t,t+1)})-\set{(i,j)})\\
%         &=
%         BASIS_b \cup B_b \cup \set{(1,t+1),\dots,(t,t+1)}
%       \end{align}
%       よって、$BASIS_b \cup B_b$は帰納法の仮定で$S(G_b)$の基底になっているから、
%       $BASIS_a \cup B_a$は$S(G_a)$の基底である。
%
%       \item L.8が実行されるとき:
%       \begin{align}
%         (i,j) & \in B_b\\
%         B_a &= B_b - \set{(i,j)}\\
%         TRASH_a  &= TRASH_a \cup \set{(i,j)}\\
%         BASIS_a &= BASIS_b\\
%         G_a &= G_b
%       \end{align}
%       $BASIS_b \cup B_b$が$S(G)$の基底で、L.7より$f_i,f_j \in B_b$のどちらとも異なる
%       $f_k \in G_b$が存在して、
%       $f_i,f_j,f_k$は相異なり、$[i,k],[j,k] \in TRASH_b \cup BASIS_b$となり、
%       さらに$\LT(f_k)|\LCM(\LT(f_i),\LT(f_j))$となるので、
%       命題10より、$(BASIS_b \cup B_b)-\set{(i,j)}$もまた$S(G_b)$の基底である。
%       \begin{align}
%         BASIS_a \cup B_a
%         &=
%         BASIS_b \cup (B_b - \set{(i,j)})\\
%         &=
%         (BASIS_b \cup B_b) - \set{(i,j)}
%       \end{align}
%       なので、$BASIS_a \cup B_a$も$S(G_b)$の基底であり、$G_a = G_b$なので、
%       $S(G_a)$の基底である。
%     \end{itemize}
%     \item $(i,j)\in BASIS$について、$S(f_i,f_j)\toG 0$となる:
%     $G$は増加し続けるので、$(i,j)\in BASIS_b$について$S(f_i,f_j)\to_{G_b} 0$ならば、
%     $S(f_i,f_j) \to_{G_a} 0 $である。$BASIS$が変化しない場合はこれより明らかなので、
%     L.17が実行されるときだけを考えればよい。
%     \begin{align}
%       B_a &= B_b - \set{(i,j)}\\
%       BASIS_a &= BASIS_b \cup \set{(i,j)}
%     \end{align}
%     である。新たに加わった$(i,j)$について、$S(f_i,f_j)\to_{G_a} 0$を示せばよい。
%     \begin{itemize}
%       \item L.10の条件が満たされないとき:
%       $f_i,f_j \in G_b = G_a$であり、$f_i,f_j$が互いに素なので、
%       定理より$S(f_i,f_j) \to_{G_a} 0$である。
%       \item L.10の条件は満たされるがL.12の条件を満たさないとき:
%       $\ovd{S(f_i,f_j)}{G_a}= \ovd{S(f_i,f_j)}{G_b}=0$なので、
%       割り算アルゴリズムより$S(f_i,f_j) \to_{G_a} 0$である。
%       \item L.10の条件が満たされ、L.12の条件も満たされるとき:
%       \begin{align}
%         G_a &= G_b \cup \set{\ovd{S(f_i,f_j)}{G_b}}\\
%         B_a &= B_b \cup \set{(1,t+1),\dots,(t,t+1)}
%       \end{align}
%       $G_b$での割り算を考え、
%       \begin{align}
%         S(f_i,f_j) &= \sum_{I=1}^t a_I f_I + \ovd{S(f_i,f_j)}{G_b}
%       \end{align}
%       である。しかしこれは$G_a$では既に割り切った形になっているので、
%       $\ovd{S(f_i,f_j)}{G_a} = 0$であり、割り算アルゴリズムいより
%       $S(f_i,f_j) \to_{G_a} 0$である。
%     \end{itemize}
%     \item 停止すること:
%     停止しなかったとすると、L.17で$B$を減らし続けていることから、
%     L.14が無数に実行されなければならないことがわかる。これは、
%     L.13より$G$および$G$のイデアルが無限に増大し続けることを意味するが、
%     これは多項式環のACCに反する。
%     \item 正しい答えが得られること:
%     停止時点で$BASIS = BASIS\cup B$が$S(G)$の基底であり、さらに
%     $(i,j) \in BASIS \implies S(f_i,f_j)_\toG 0$なので、
%     「ある斉次基底について、それらすべて$G$との積がすべて$G$を法として0に簡約されるならグレブナ基底」より(斉次基底として$BASIS$、$G$として$G$をとる。)、$G$はグレブナ基底である。
%   \end{itemize}
%
\end{myproof}
...
もう1回。
$I=\gen{f_1,\dots,f_s}$を多項式イデアルとする。
$I$のグレブナ基底は、次のアルゴリズムによって有限回のステップで構成することができる。
入力:$F=(f_1,\dots,f_s)$。出力:$G$、これは$I=\gen{f_1,\dots,f_s}$のグレブナ基底。
\begin{algorithm}[H]
  \begin{algorithmic}[1]
    \STATE{$B\defeq \set{(i,j); 1\le i < j \le s}$}
    \STATE{$G\defeq F$}
    \STATE{$t\defeq s$}
    \WHILE{$B\neq \emptyset$}
    \STATE{Select $(i,j)\in B$}
    \IF{$\LCM(\LT(f_i),\LT(f_j))\neq \LT(f_i)\LT(f_j)$ かつ
    $\Criterion(f_i,f_j,B) = false$}
    \STATE{$S\defeq \ovd{S(f_i,f_j)}{G}$}
    \IF{$S\neq 0$}
    \STATE{$t\defeq t+1$}
    \STATE{$f_t \defeq S$}
    \STATE{$G\defeq G\cup \set{f_t}$}
    \STATE{$B\defeq B\cup \set{(a,t); 1\le a \le t-1}$}
    \ENDIF{}
    \STATE{$B\defeq B-\set{(i,j)}$}
    \ENDIF{}
    \ENDWHILE{}
  \end{algorithmic}
\end{algorithm}
...
\end{align}
としておく。
ここで、$\Criterion(f_i,f_j,B)$は次の条件がみたされるとき真である:
\begin{align}
  \Exists{k: 1\le k \le t,\, kはiでもjでもない}
  \begin{cases}
    [i,k] \in \set{(a,b); 1 \le a < b \le t}-B\\
    [j,k] \in \set{(a,b); 1 \le a < b \le t}-B\\
    \LT(f_k)|\LCM(\LT(f_i),\LT(f_j))
  \end{cases}.
\end{align}

\begin{myproof}
  \begin{itemize}
    \item アルゴリズムの各段階で、$B$は次の性質を持つ:
    \begin{align}
      (i,j) \in \set{(a,b); 1\le a < b \le t}-B
      \implies
      S(f_i,f_j)\toG 0 \quad
      または
      \quad
      \Criterion(f_i,f_j,B).
    \end{align}
    初期では$B$が全体なので、自明に「性質」は成立する。

    途中でも大丈夫なことを示す。$(I,J)\in \set{(a,b); 1\le a < b \le t_a}-B_a$とする。$(I,J)\in B_b$と$(I,J)\notin B_b$のときで場合分けする。
    \begin{enumerate}[label=(場合\arabic*)]
      \item $(I,J)\in B_b$のとき:
      $(I,J)\in B_b$でかつ$(I,J)\notin B_a$なので、アルゴリズムを調べると
      L.5で選択された$(i,j)$は$(I,J)$である。
      次の場合分けをする。
      \begin{enumerate}[label=(場合\roman*)]
        \item $\LCM(\LT(f_I),\LT(f_J))=\LT(f_I)\LT(f_J)$のとき:
        $G_b = G_a$なので$f_I,f_J \in G_a$であり、
        先の定理より、$S(f_i,f_j)\toG 0$である。
        \item 上をみたさず、$\Criterion(f_I,f_J,B_b)$のとき:
        $B_a = B_b -\set{(i,j)}$であり、$t_a = t_b$である。
        $B$の補集合は広がるだけなので、あきらかに$\Criterion(f_I,f_J,B_a)$が満たされる。

        正確に言うと、$t=t_b=t_a$として、
        $\Criterion(f_I,f_J,B_b)$なので、$1\le K \le t$で$k$は$I$でも$J$でもなく、
        \begin{align}
          [I,K] \in \set{(a,b); 1\le a < b \le t}-B_b\\
          [J,K] \in \set{(a,b); 1\le a < b \le t}-B_b\\
          \LT(f_K)|\LCM(\LT(f_I),\LT(f_J))
        \end{align}
        となる$K$がある。
        $B_a \subset B_b$なので、
        \begin{align}
          [I,J] \in (\set{(a,b); 1\le a < b \le t}-B_b) \subset
          (\set{(a,b); 1\le a < b \le t}-B_a)\\
          [K,J] \in (\set{(a,b); 1\le a < b \le t}-B_b) \subset
          (\set{(a,b); 1\le a < b \le t}-B_a)\\
        \end{align}
        であり、$K$がそのまま使えて、$\Criterion(f_I,f_J,B_a)$が成立する。
        \item 上をみたさず、$S=0$のとき:
        $G_b = G_a$でかつ、$\ovd{S(f_I,f_J)}{G_b} = 0$がわかったので、
        $\ovd{S(f_I,f_J)}{G_a}=0$であり、
        割り算アルゴリズムより$S(f_I,f_J) \to_{G_a} 0$である。
        \item 上をみたさず、$S\neq 0$のとき:
        $t_a = t_b + 1$である。
        $G_a = G_b \cup \set{f_{t_a}}$である。
        $S(f_I,f_J)$を$G_b$で割り、
        \begin{align}
          S(f_I,f_J) = h_1 f_1 + \dots + h_{t_b} f_{t_b} + \ovd{S(f_I,f_J)}{G_b}
        \end{align}
        と、商$(h_1,\dots,h_{t_b})$と余り$\ovd{S(f_I,f_J)}{G_b}$が得られたとする。
        $S(f_I,f_J)$を$G_a$で割ると、商$(h_1,\dots,h_{t_b},1)$で割り切れる。
        よって、$\ovd{S(f_I,f_J)}{G_a} = 0$であり、
        $\ovd{S(f_I,f_J)}{G_a} \to_{G_a}0$である。
      \end{enumerate}
      \item $(I,J)\notin B_b$のとき:
      \begin{align}
        (I,J) \in \set{(a,b); 1\le a < b \le t_b}-B_b
      \end{align}
      である。よって、「性質」より、
      \begin{align}
        S(f_{I},f_{J})\to_{G_b} 0 \quad
        または
        \quad
        \Criterion(f_I,f_J,B_b)
      \end{align}
      が満たされる。
      \begin{enumerate}[label=(場合\roman*)]
        \item $S(f_I,f_J)\to_{G_b}0$のとき:
        $G$は増えるかそのままなので、$G_b \subset G_a$であり、
        $S(f_I,f_J)\to_{G_a}0$となる。
        \item 上がみたされず、$\Criterion(f_I,f_J,B_b)$のとき:
        L.5で$(i,j)\in B_b$が選ばれたとする。L.12が実行されるときには、$t_a = t_b + 1$であり、
        \begin{align}
          \set{(a,b); 1\le a < b\le t_a}-B_a
          &=
          \set{(a,b);1\le a < b \le t_a}
          -
          ((B_b \cup \set{(a,t_a); 1\le a \le t_b})-\set{(i,j)})\\
          &=
          \set{(a,b); 1\le a < b \le t_b}-(B_b - \set{(i,j)})\\
          &=
          (\set{(a,b); 1\le a < b \le t_b}-B_b)\cup \set{(i,j)}
        \end{align}
        となる。

        $B$の補集合が膨らむだけなので、$\Criterion(f_I,f_J,B_a)$も満たされる。
      \end{enumerate}
    \end{enumerate}
    よって、どの場合でも
    \begin{align}
      S(f_I,f_J)\to_{G_a}0 \quad または \quad \Criterion(f_I,f_J,B_a)
    \end{align}
    がみたされる。
    \item $B=\emptyset$となったとき(アルゴリズムが停止したとき)$G$はグレブナ基底である:
    \begin{align}
      \cal I &= \set{(i,j); 1\le i < j \le t,\quad \Criterion(f_i,f_j,B)は選択されたときにfalseである。}\\
      \cal S &= \set{S_{ij}; (i,j)\in \cal I}
    \end{align}
    と定義する。そして、$\cal S$が「$S_{ij}G = S(f_i,f_j) \to_G 0$がすべての
    $S_{ij} \in \cal S$に対して成立する」という性質を持つ$S(G)$の基底であることを示す。

    先に「アルゴリズムの各段階で、$B$は『性質』を持つ」ことを示していたので、
    \begin{align}
      (i,j)\in \set{(a,b); 1\le a < b \le t} \implies
      (S(f_i,f_j) \to_G 0\quad
      または
      \quad
      \Criterion(f_i,f_j,\emptyset))
    \end{align}
    が成立する。よって、任意の$S_{ij}\in \cal S$について、$S_{ij}$
    上の証明中で、ひとたび$\Criterion(f_i,f_j,B)$がfalseになったなら、
    以降ずっとfalseになることがわかっている(「$B$の補集合は膨らむだけ」)ので、すべての$(i,j)\in \cal I$について
    $S(f_i,f_j)\to_{G}0$となる。

    あとは、$\cal S$が$S(G)$の基底になることを示せばよい。
    $B$から取り除かれた順に添字のペアを並べ、その逆順のリストを考える。
    この時点で、リストは$\set{(a,b); 1\le a < b\le t}$全てを含んでいるので、
    ここから$S_{ij}$たちを作ると
    $S(G)$の基底になっている。
    このリストを先頭から走り、$(i,j)$のその時点での$B$について、
    $\Criterion(f_i,f_j,B)$がtrueであるとき、その$(i,j)$をリストから取り除くという操作を最後までやる。
    リストを逆順にしてあるので、$\Criterion(f_i,f_j,B)$が真であることから得られる$k$について、$[i,k],[j,k]$がリストのその先に存在し、先の定理より除去しても
    リストは$S(G)$の基底を作り続ける。
    そして、リストの取り除きかたより、操作が終了したときにはリストに残っているものは$\cal I$と一致する。よって、$\cal I$は$S(G)$の基底となる。

    よって、、$\cal S$は$S_{ij}G = S(f_i,f_j) \to_G 0$がすべての
    $S_{ij} \in \cal S$に対して成立する」という性質を持つ$S(G)$の基底である。
    よって、「$G$の斉次基底で、そのすべての基底$S$について$SG \to_G 0$となる」もの
    である$\cal S$がみつかったので、$G$はグレブナ基底である。
    $G$には$f_1,\dots,f_s$たちのあまりしか追加していないので、
    $G$は$\gen{f_1,\dots,f_s}$のグレブナ基底になっている。

  \item 停止すること:
  もしも停止しなかったとすると、L.14で毎回$B$が減少することから
  L.12が無数回実行されなければならず、それに伴なってL.11が無数に実行されなければならないが、これは$\gen{\LT(G)}$の無限上昇列を生んでしまい、多項式環がネーター環であることに違反する。
  \end{itemize}
\end{myproof}
